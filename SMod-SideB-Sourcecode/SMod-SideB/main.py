...
pygame.display.set_icon(pygame.image.load(resource_path("icon.png")).convert_alpha())
...
celltypes = {
    ...
    "one directional": {"desc": "Can only be pushed on the indicated axes."},
    "two directional": {"desc": "Can only be pushed on the indicated axes."},
    "random push": {"desc": "Has a 1/2 chance to be pushable every tick."},
    "three directional": {"desc": "Can only be pushed on the indicated axes."},
    ...
    "random rotator": {"desc": "Rotates either 90 degrees clockwise or counterclockwise."},
    ...
    "crimson": {"desc": "Turns all surrounding cells into itself."},
    ...
    "straight diverger": {"desc": "Diverger but straight."},
    "freezer": {"desc": "Stops adjacent cells from updating for the rest of the tick."},
    "player": {"desc": "Mover that is controlled with WASD or arrow keys."},
    "mirror": {"desc": "Swaps the cells on the left and right of it."},
...
subcategories = {
    ...
    "trashes": ["trash"],
    "enemies": ["enemy"],
    "divergers": ["curve diverger","straight diverger"],
    "redirectors": ["redirector"],
    "effect givers": ["freezer", "thawer"],
    "gears": ["cw gear", "ccw gear"],
    "sideb": ["mirror", "crimson"], # Added SideB custom cells
}
...
categories = {
    ...
    "Side B": [[subcategories["sideb"]], images["mirror"]],
...
def update_category_buttons():
    ...
    for i, (cat_name, cat_data) in enumerate(categories.items()):
        cat_icon = cat_data[-1]
        bx, by = i * 70 + 20, 20  # Row 1 (Bottom row)
...
def update_subcategory_buttons():
    ...
    if current_category in categories:
        subcat_list = categories[current_category][0]  # Always gets the subcategory list
        for j, subcat in enumerate(subcat_list):
            ...
            for s_name, s_val in subcategories.items():
                if s_val == subcat:
                    ...
                    break
            if subcat_name:
                sbx, sby = j * 70 + 20, 90  # Row 2 (Spawns above Row 1)
...
def update_cell_buttons():
    ...
    if current_subcategory and current_subcategory in subcategories:
        ...
        for k, cell_name in enumerate(cell_list):
            sbx, sby = k * 70 + 20, 160  # Row 3 (Shifted 140px above row 1)
            add_ui(ui.ImageButton(sbx, sby, 60, 60, images[cell_name],
...
def draw_infobox():
    ...
    if btn_hovering is not None and btn_hovering.tag[0] == "Cell Button":
        cell_key = btn_hovering.tag[1].lower()
        if cell_key not in celltypes:
            return
